...
```bash
cd server
docker buildx build --platform linux/amd64,linux/arm64/v8 -t owenrbee/hands-free-api:latest  -t owenrbee/hands-free-api:1.5.0 --push .
```

**Frontend:**
```bash
cd frontend
docker buildx build --platform linux/amd64,linux/arm64/v8 -t owenrbee/hands-free-ux:latest -t owenrbee/hands-free-ux:1.5.0 --push .
```
